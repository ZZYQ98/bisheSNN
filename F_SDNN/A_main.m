...
retrain_params=struct('a_minus_r', 0.03, 'a_plus_r', 0.05, 'a_minus_p', 0.07, 'a_plus_p', 0.01,...
                     'tao_minus_r',40, 'tao_plus_r',20, 'tao_minus_p', 80, 'tao_plus_p', 10,'retrain_iter',900);
%-------------------------------------根据输入参数创建网络SDNN_net----------------------------------------------------
%网络结构初始化
...
if set_weights==1
   %读取从文件中读取权值
   weights_path_list='weights_4_13_train_SDNN.mat';    %权值读取路径
   ...
   weights=weights.weights;
    ...
    save('weights_4_13_train_SDNN.mat','weights');
end   
%对于未进行relearn的权值进行分类-----------------------------------------------------------------------------------
...
%权值存储
if save_weights==1
    save('weights_4_13_retrain_SDNN.mat','weights');
end
%----------------------------------特征判别与输出特征--------------------------------------------------------------------
% % %特征判别
% 
