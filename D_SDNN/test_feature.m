...
X_test=zeros(num_layers,n_featuers);
...
%  X_train: Training features of size (N, M) where N is the number of training samples and M is the number of maps in the last layer
       ...
       for ii=1:num_img_train
        ...
        features_test=layers{num_layers-1}.V;%最后一层V的值是一个1*1*D的三维矩阵
        features_test1=max(features_test,[],1);
        features=max(features_test1,[],2);
