function [ weights,K_STDP_post,weight_STDP_flag ] =STDP_negative(S,s,K_STDP_post,stride,pad,weight_STDP_flag,weights,deta_STDP_minus)
%UNTITLED2 此处显示有关此函数的摘要
global STDP_Flag_n
[~,~,Ds]=size(s);
[H,W,D]=size(S);%D 与WD的大小相同
[HH,WW,WD]=size(weights); 
                          ...
                          for I=1:WW
                             for J=1:HH
                                 if local_image(I,J,L)==1 && STDP_Flag_n(k)>0
                                     weights(I,J,k)=weights(I,J,k)-deta_STDP_minus(K_STDP_post(i,j,k));
