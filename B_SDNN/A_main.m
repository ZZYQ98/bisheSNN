...
l3=struct('type', 'pool', 'num_filters', 1, 'filter_size', 17, 'th', 0., 'stride', 16);
l4=struct('type', 'conv', 'num_filters',10, 'filter_size', 7, 'th', 3);
l5=struct('type', 'pool', 'num_filters', 1, 'filter_size', 10, 'th', 0., 'stride', 1);
learnable_layers=[2,4];
network_params={l1,l2,l3,l4,l5};
weight_params=struct('mean',0.8,'std',0.05);%定义权值初始化参数
max_learn_iter=[0,30,0,50,0];
...
STDP_time_pre=30;%增强型STDP权值更新
STDP_time_post=40;%抑制性
%-------------------------------------根据输入参数创建网络SDNN_net----------------------------------------------------
%网络结构初始化
...
STDP_time_minus=40;
STDP_time_plus=30;
deta_STDP_minus=deta_STDP(0.03,STDP_time_minus,40);  %作用窗长度为STDP_time_minus=40，时长数为40
deta_STDP_plus=deta_STDP(0.05,STDP_time_plus,20);    %作用窗长度为STDP_time_plus=30，时长数为20
%——————————————————输入脉冲是否经过滤波得到————————————————————————————————————
% 
if DoG==1
