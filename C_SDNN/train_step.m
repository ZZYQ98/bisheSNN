...
STDP_Flag_p=ones(1,D);
STDP_Flag_n=ones(1,D);
...
%     K_STDP_pre=layers{learning_layer-1}.K_STDP;
    ...
    for i=2:learning_layer    %
        ...
        if strcmp( network_struct{i}.Type,'conv' )%该层为卷积层时  
           ...
           layers{i-1}.K_STDP=K_STDP_update(s,layers{i-1}.K_STDP);%K_STDP_pre 进行了权值更新
           conv_step4(i,s_pad,stride,th);
 ...
 if check_Spike
    ...
    STDP_negative(layers{learning_layer}.S,layers{learning_layer-1}.S,layers{learning_layer}.K_STDP  ,learning_layer,network_struct{learning_layer}.stride,network_struct{learning_layer}.pad) ;
      ...
      for k=1:D
          for i=1:H
              for j=1:W
                  if weight_STDP_flag(i,j,k)==0 && layers{learning_layer}.K_inh(i,j)~=0
                      weights{learning_layer}(i,j,k)=weights{learning_layer}(i,j,k)-a_minus(learning_layer);
                      if weights{learning_layer}(i,j,k)<1e-6
                          weights{learning_layer}(i,j,k)=1e-6;
                      end
                  end
              end
          end
   ...
   if sum(sum(layers{learning_layer}.K_inh))==0%||sum(STDP_Flag_p)+sum(STDP_Flag_n)==0
      continue 
   end
end
